...
1. **Code GLM in STAN**
  ...
  - figure out how to incorporate .stan into RMarkdown
2. **research GLMs**
  - error
  - alpha
  - beta
...
--**since production is a constant, we may want to switch to a different distribution**
  - log normal could be a better fit
...
- however, this can still be coded in STAN --> but is it Bayesian?
...
**Loading Data for STAN Model**
- to fit data to model - how does data need to be prepared?
- it's not prepared right now -- maybe because catches and aquaculture aren't binomial yet
```{r}
#From intro to STAN example
fit <- stan(file = 'FishBaeSTAN.stan', data = salmon_lobster, 
            iter = 1000, chains = 4)


##From Log Normal Example
#load libraries
library(rstan)
library(coda)
set.seed(20151204)
