...
3. For a GLM, the book has us generate data. We don't have to do this since we have our own data correct?
...
modeltestBAY <- glm(general_name ~ quantity * year + I(year^2) + I(year^3), family = "poisson", data = fao_salmon_lobster)
summary(modeltestBAY)

modeltestFREQ <- glm(general_name ~ quantity * year, family = "binomial", data = fao_salmon_lobster)
