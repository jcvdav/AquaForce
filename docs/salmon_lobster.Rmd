...
```{r}
model7 <- lm(log(landings) ~  group * as.factor(years_since) + country + as.factor(year) + aquaculture + species, data = data_for_model_salmon_lobster)
```

And a similar approach, but using years since technological advance instead

```{r}
model8 <- lm(log(landings) ~  group * as.factor(years_since_tech_improve) + country + as.factor(year) + aquaculture + species, data = data_for_model_salmon_lobster)
```
