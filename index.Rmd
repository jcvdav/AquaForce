...
rbind(select_treated, select_control) %>% 
  select(group, family, total, rank) %>% 
  View()

# number of species in control families

FAO_sp_codes %>% filter(family %in% select_control$family) %>% group_by(family, species) %>% tally() %>% select(-n) %>% group_by(family) %>% tally() %$% sum(n)

# number of species in treated families

FAO_sp_codes %>% filter(family %in% select_treated$family) %>% group_by(family, species) %>% tally() %>% select(-n) %>% group_by(family) %>% tally() %$% sum(n)
