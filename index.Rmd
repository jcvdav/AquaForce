...
fao_salmon_lobster  %>%
  ...
  summarize(quantity = sum(quantity)) %>% 
  filter(!(general_name == "Lobster" & type == "Aquaculture")) %>% 
  rename(Species = general_name,
         Type = type) %>% 
  mutate(`Source-Type` = paste(source, Type, sep = "-")) %>% 
  ggplot(aes(x = year, y = quantity/1000000, color = `Source-Type`, linetype = Species, group = paste(Type, source, Species))) +
  geom_line(size = 1) +
  scale_color_brewer(palette = "Set1") +
