...
fao_salmon_lobster_aqua <- filter(fao_salmon_lobster, source == "Aquaculture") %>% 
  group_by(year) %>% 
  summarize(Salmon = log(sum(quantity, na.rm = T))) %>% 
  mutate(Lobster = 0) %>% 
...
model1 <- lm(log(quantity) ~ aquaculture, fao_salmon_lobster_model)

model2 <- lm(log(quantity) ~ factor(year) + aquaculture, fao_salmon_lobster_model)

model3 <- lm(log(quantity) ~ factor(year) + aquaculture + species, fao_salmon_lobster_model)

model4 <- lm(log(quantity) ~ factor(year) + aquaculture + species + iso, fao_salmon_lobster_model)
